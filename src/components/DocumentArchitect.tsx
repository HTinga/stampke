    ...
    <div className="flex h-screen bg-[#21262d] overflow-hidden">
      <AnimatePresence>
        {showTemplatePicker && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-[#21262d] overflow-y-auto py-12 px-4 md:px-8"
          >
            <div className="max-w-7xl mx-auto">
              <div className="mb-16 text-center">
                ...
                <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter mb-6">
                  Select a <span className="text-[#58a6ff]">Template.</span>
                </h1>
                <p className="text-xl text-[#8b949e] max-w-2xl mx-auto font-medium">
                  Choose from our library of professional business documents to start your project.
                </p>
              </div>

              <div className="flex flex-col md:flex-row gap-6 items-center justify-between bg-[#161b22] p-8 rounded-[40px] border border-[#21262d] shadow-sm mb-12">
      ...
      <div className="w-20 bg-[#161b22] border-r border-[#30363d] flex flex-col items-center py-8 gap-8 z-20">
        <div 
          ...
          className="bg-[#1f6feb] text-white p-3 rounded-2xl shadow-lg shadow-black mb-4 cursor-pointer hover:scale-110 transition-transform"
        >
          <Grid size={24} />
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-20 bg-[#161b22] border-b border-[#30363d] px-8 flex items-center justify-between z-10">
          <div className="flex items-center gap-4">
            <div className="bg-[#21262d] p-2 rounded-xl text-[#8b949e]">
              <FileText size={20} />
            </div>
            <input 
              type="text" 
              ...
              className="text-xl font-black text-white bg-transparent border-none outline-none focus:ring-0 w-64"
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              ...
              className="bg-[#1f6feb] text-white px-6 py-3 rounded-2xl font-black text-sm flex items-center gap-2 hover:bg-[#30363d] transition-all shadow-xl shadow-black active:scale-95 disabled:opacity-50"
            >
        ...
        {/* Canvas Area */}
        <main className="flex-1 overflow-y-auto p-12 flex flex-col items-center gap-12 bg-[#21262d] scroll-smooth custom-scrollbar">
          <div ref={canvasRef} className="flex flex-col items-center gap-12">
            {pages.map((page, index) => (
              <div 
                key={page.id}
                onClick={() => setActivePageId(page.id)}
                className={`a4-page relative bg-[#161b22] shadow-2xl border transition-all duration-300 ${activePageId === page.id ? 'border-[#1a5cad] ring-4 ring-[#1f6feb]/10' : 'border-[#30363d]'}`}
                style={{ width: '210mm', minHeight: '297mm', padding: '20mm' }}
              >
                {/* Page Number Badge */}
                <div className="absolute -left-16 top-0 bg-[#161b22] border border-[#30363d] px-4 py-2 rounded-xl font-black text-[#8b949e] shadow-sm">
                  {index + 1}
                </div>
          ...
          >
            <div className="w-12 h-12 bg-[#161b22] border-2 border-dashed border-[#aaccf2] rounded-2xl flex items-center justify-center text-[#8b949e] group-hover:border-[#1a5cad] group-hover:text-[#58a6ff] transition-all">
              <Plus size={24} />
            </div>
            <span className="text-sm font-black text-[#8b949e] group-hover:text-[#58a6ff] uppercase tracking-widest">Add New Page</span>
          </button>
        </main>
      </div>
    </div>
...
function SidebarItem({ icon, label, onClick }: { icon: React.ReactNode, label: string, onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
      className="group relative flex items-center justify-center w-12 h-12 rounded-xl hover:bg-[#21262d] text-[#8b949e] hover:text-[#58a6ff] transition-all"
    >
      {icon}
    ...
    >
      ...
      <AnimatePresence>
        {isHovered && (
          ...
          >
            <button onClick={onRemove} className="p-2 bg-[#161b22] border border-[#30363d] text-red-500 rounded-lg shadow-sm hover:bg-red-50 transition-all">
              <Trash2 size={14} />
            </button>
            <button className="p-2 bg-[#161b22] border border-[#30363d] text-[#8b949e] rounded-lg shadow-sm cursor-grab active:cursor-grabbing">
      ...
      <div style={block.style}>
        {block.type === 'header' && (
          <div className="border-b-2 border-[#020b18] pb-4 mb-8">
            <input 
              ...
              className="text-2xl font-black text-white bg-transparent border-none outline-none focus:ring-0 w-full"
            />
            <input 
              type="text" 
              value={block.content.subtitle}
              onChange={(e) => onUpdate({ ...block.content, subtitle: e.target.value })}
              className="text-sm font-bold text-[#8b949e] bg-transparent border-none outline-none focus:ring-0 w-full uppercase tracking-widest"
            />
          </div>
        )}
        ...
        {block.type === 'text' && (
          <textarea 
            ...
            className="w-full bg-transparent border-none outline-none focus:ring-0 text-white leading-relaxed min-h-[50px] resize-none overflow-hidden"
      ...
      {block.type === 'table' && (
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#0d1117] border-y border-[#30363d]">
                {block.content.rows[0].map((cell: string, i: number) => (
                  <th key={i} className="p-3 text-left">
                    <input 
                      ...
                      className="w-full bg-transparent border-none outline-none focus:ring-0 font-black text-[10px] uppercase tracking-widest text-[#8b949e]"
                    />
                  </th>
                ))}
            ...
            <tbody>
              {block.content.rows.slice(1).map((row: string[], rowIndex: number) => (
                <tr key={rowIndex} className="border-b border-[#21262d]">
                  {row.map((cell: string, colIndex: number) => (
                    <td key={colIndex} className="p-3">
                      <input 
                        ...
                        className="w-full bg-transparent border-none outline-none focus:ring-0 text-sm font-medium text-[#e6edf3]"
                      />
                    </td>
                  ))}
                  <td className="p-3">
          ...
          <button 
            ...
            className="mt-4 flex items-center gap-2 text-[10px] font-black text-[#58a6ff] uppercase tracking-widest hover:text-blue-700 transition-colors"
          >
      ...
      {block.type === 'image' && (
        <div className="relative group/img">
          ...
          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/img:opacity-100 transition-all flex items-center justify-center gap-4">
            <button className="bg-[#161b22] text-white px-4 py-2 rounded-lg font-bold text-xs">Change Image</button>
          </div>
        </div>
      )}

      {block.type === 'signature' && (
        <div className="mt-12 pt-8 border-t border-[#30363d] w-64">
          <div className="h-16 flex items-end justify-center mb-2">
            <p className="font-serif italic text-2xl text-[#8b949e]">Signature</p>
          </div>
          <input 
            type="text" 
            value={block.content.name}
            onChange={(e) => onUpdate({ ...block.content, name: e.target.value })}
            className="w-full bg-transparent border-none outline-none focus:ring-0 text-sm font-black text-white text-center uppercase tracking-widest"
          />
          <p className="text-[10px] font-bold text-[#8b949e] text-center uppercase tracking-widest mt-1">
            {block.content.date}
          </p>
        </div>
      )}
      ...
      {block.type === 'stamp' && (
        <div className="w-32 h-32 border-4 border-[#58a6ff]/30 rounded-full flex items-center justify-center p-4 rotate-[-15deg] opacity-60">
          <p className="text-center text-[10px] font-black text-[#58a6ff] uppercase tracking-widest leading-tight">
            {block.content.text}
          </p>
        </div>
      )}
      ...
      {block.type === 'footer' && (
        <div className="mt-auto pt-8 border-t border-[#21262d] flex justify-between items-center text-[10px] font-black text-[#8b949e] uppercase tracking-widest">
