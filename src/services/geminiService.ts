...
// Analyze a document's text content via AI to provide summaries and translations
export const analyzeDocumentText = async (text: string, task: 'summarize' | 'translate'): Promise<string> => {
  if (!apiKey) {
    return task === 'summarize' 
      ? "AI Summarization (Simulated): This document appears to be a professional contract or agreement. Key points include terms of service, payment schedules, and liability clauses. [Configure API_KEY for real AI analysis]"
      : "AI Translation (Simulated): Nakala hii ni makubaliano ya kitaalamu yanayohusu huduma, malipo, na majukumu. [Configure API_KEY for real AI analysis]";
  }

  try {
    const ai = new GoogleGenAI({ apiKey });
    const prompt = task === 'summarize'
      ? `Provide a professional, bulleted summary of this document. Focus on key dates, obligations, and parties involved:\n\n${text.slice(0, 10000)}`
      : `Translate the following text into Swahili (professional tone):\n\n${text.slice(0, 10000)}`;
    ...
    console.error('[AI Document Analysis]', e);
