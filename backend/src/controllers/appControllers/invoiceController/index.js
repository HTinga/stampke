...
methods.sendReminder = async (req, res) => {
  ...
  const dueDate = invoice.expiredDate
    ? new Date(invoice.expiredDate).toLocaleDateString('en-KE', { day: 'numeric', month: 'long', year: 'numeric' })
    : 'ASAP';
  ...
  try {
    ...
    await resend.emails.send({
      ...
      html: '<div style="font-family:sans-serif;max-width:480px"><h2>Payment Reminder</h2>'
          ...
          + '<p style="color:#888;font-size:12px">Reminder #' + (invoice.reminderCount + 1) + ' sent via Tomo</p></div>',
    });
  } catch (e) {
