...
const EditorControls: React.FC<EditorControlsProps> = ({ config, onChange, userTier, onUpgrade, t }) => {
  ...
  return (
    <div className="space-y-4 pb-20 lg:pb-0">
      ...
      {controlBlock(t.sw === 'Rekebisha Muhuri' ? 'UANDISHI' : "Typography", <Type size={14}/>, (
      ...
      {controlBlock(t.sw === 'Rekebisha Muhuri' ? 'UKUBWA NA MPANGILIO' : "Sizing & Adjustments", <Maximize size={14}/>, (
      ...
      {controlBlock(t.sw === 'Rekebisha Muhuri' ? 'MAANDISHI' : "Text Content", <Sliders size={14}/>, (
      ...
      {controlBlock(t.sw === 'Rekebisha Muhuri' ? 'RANGI NA ATHARI' : "Ink & Effects", <PenTool size={14}/>, (
      ...
      <div className="grid grid-cols-2 gap-3">
        <button onClick={() => isPro ? setShowSigPad(true) : onUpgrade()} className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 flex flex-col items-center gap-2 hover:bg-blue-50 transition-all group">
           ...
           <span className="text-[9px] font-black uppercase tracking-widest dark:text-slate-300">{t.signOverlay}</span>
        ...
        <button onClick={() => isPro ? onChange({ includeCertificate: !config.includeCertificate }) : onUpgrade()} className={`p-4 rounded-2xl border flex flex-col items-center gap-2 transition-all ${config.includeCertificate ? 'bg-emerald-50 border-emerald-200' : 'bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800'}`}>
           ...
           <span className="text-[9px] font-black uppercase tracking-widest dark:text-slate-300">{t.authCert}</span>
           {!isPro && <Lock size={10} className="text-amber-500" />}
        </button>
      </div>
    </div>
