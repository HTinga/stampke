    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-[#8b949e] font-medium">Stay updated with the latest firm news, policies, and events.</p>
        </div>
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="flex items-center gap-4 mb-2 overflow-x-auto pb-2 scrollbar-hide">
            {['All', 'Policy', 'Events', 'Urgent', 'General'].map(cat => (
              <button key={cat} className="px-6 py-2 bg-[#161b22] dark:bg-[#161b22] border border-[#21262d] dark:border-[#30363d] rounded-xl text-xs font-black uppercase tracking-widest hover:border-[#aaccf2] transition-all whitespace-nowrap">
                {cat}
              </button>
            ))}
          </div>
          ...
          <div className="space-y-6">
            {announcements.map(ann => (
              <motion.div 
                ...
                className="bg-[#161b22] dark:bg-[#161b22] p-10 rounded-[56px] border border-[#21262d] dark:border-[#30363d] shadow-sm hover:shadow-md transition-all group"
              >
                <div className="flex items-start justify-between mb-8">
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${ann.category === 'Urgent' ? 'bg-rose-50 text-rose-600' : ann.category === 'Policy' ? 'bg-[#21262d] text-[#58a6ff]' : 'bg-[#0d1117] text-[#e6edf3]'}`}>
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {!ann.isRead && <div className="w-2 h-2 rounded-full bg-[#1f6feb]" />}
                      </div>
                      <p className="text-[10px] font-black uppercase text-[#8b949e] tracking-widest">{ann.author} • {ann.date}</p>
                    </div>
                  </div>
                  <button className="p-2 text-[#8b949e] hover:text-[#58a6ff] transition-all"><Bookmark size={20} /></button>
                </div>

                <p className="text-[#e6edf3] dark:text-[#8b949e] font-medium leading-relaxed mb-8">
                  {ann.content}
                </p>

                <div className="flex items-center justify-between pt-8 border-t border-[#f0f6ff] dark:border-[#30363d]">
                  <div className="flex items-center gap-6">
                    <button className="flex items-center gap-2 text-xs font-black text-[#8b949e] hover:text-rose-500 transition-all">
                      <Heart size={16} /> {ann.likes}
                    </button>
                    <button className="flex items-center gap-2 text-xs font-black text-[#8b949e] hover:text-[#58a6ff] transition-all">
                      <MessageSquare size={16} /> {ann.comments}
                    </button>
                    <button className="flex items-center gap-2 text-xs font-black text-[#8b949e] hover:text-[#e6edf3] transition-all">
                      <Share2 size={16} /> Share
                    </button>
                  </div>
                  <button className="flex items-center gap-2 text-xs font-black text-[#58a6ff] uppercase tracking-widest group">
            ...
            <div className="space-y-6">
              {[
                ...
                <div key={i} className="flex gap-4">
                  <div className="w-12 h-12 bg-[#0d1117] dark:bg-[#21262d] rounded-2xl flex flex-col items-center justify-center">
                    <span className="text-[10px] font-black text-[#58a6ff] uppercase">{event.date.split(' ')[0]}</span>
                    <span className="text-sm font-black">{event.date.split(' ')[1].replace(',', '')}</span>
                  </div>
                  <div>
                    <h4 className="text-sm font-bold">{event.title}</h4>
                    <p className="text-[10px] font-black uppercase text-[#8b949e] tracking-widest">{event.date.split(',')[1].trim()}</p>
                  </div>
                </div>
              ))}
            </div>
