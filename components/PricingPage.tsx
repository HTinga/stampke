...
import { Check, Zap, Crown, ArrowRight, Smartphone, CreditCard, X, Loader2, Shield } from 'lucide-react';
import { PLANS, startMpesaPayment, startCardPayment, checkMpesaStatus, formatPrice } from '../src/payments';
...
export default function PricingPage({ userEmail = '', userName = '', currentPlan = 'trial', onClose }: PricingPageProps) {
  ...
  const [mpesaFlow, setMpesaFlow]   = useState<{ planId: 'pro' | 'enterprise'; step: MpesaStep; reqId?: string } | null>(null);

  // ── Card (IntaSend hosted) ──────────────────────────────────────────────────
  const handleCard = async (planId: 'pro' | 'enterprise') => {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-16">
      <div className="text-center mb-10 relative">
        ...
        <p className="text-[#8b949e]">eSign &amp; Stamps are <strong className="text-white">always free</strong>. Unlock everything else.</p>
      </div>

      <div className="grid md:grid-cols-3 gap-5 mb-6">
        {planList.map(plan => {
          const isCurrent = currentPlan === plan.id || (plan.id === 'free' && currentPlan === 'trial');
            ...
            <div key={plan.id} className={`relative flex flex-col bg-[#161b22] border rounded-2xl p-6 ${isPop ? 'border-[#1f6feb] ring-1 ring-[#1f6feb]/20' : 'border-[#30363d]'}`}>
              ...
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-1">
                  {plan.id === 'enterprise' ? <Crown size={15} className="text-purple-400" /> : plan.id === 'pro' ? <Zap size={15} className="text-[#58a6ff]" /> : <Check size={15} className="text-emerald-400" />}
              ...
              {isCurrent ? (
                ...
                <div className="w-full py-2.5 bg-[#0d1117] border border-[#30363d] text-[#8b949e] rounded-xl text-sm font-bold text-center">Always Free</div>
              ) : (
                <div className="space-y-2">
                  <button onClick={() => handleCard(plan.id as 'pro' | 'enterprise')} disabled={!!loading}
                  ...
                  <button onClick={() => { setMpesaFlow({ planId: plan.id as 'pro' | 'enterprise', step: 'phone' }); setError(''); }} disabled={!!loading}
