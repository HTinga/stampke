...
const TESTIMONIALS = [
  { name: 'Adv. Wanjiku Kamau',  role: 'Senior Partner, Kamau & Associates',       text: 'Tomo transformed how our firm handles documentation. The AI digitizer recreated our 20-year-old firm stamp perfectly in minutes.',          stars: 5, initials: 'WK', color: 'bg-[#134589]' },
...
export const LandingPage: React.FC<LandingPageProps> = ({ onGetStarted }) => {
  ...
  return (
    <div className="min-h-screen font-sans bg-[#020b18] text-white">

      {/* ── Navigation ── */}
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#020b18]/95 border-b border-[#0e3a72] backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 bg-[#134589] rounded-xl flex items-center justify-center shadow-lg shadow-[#134589]/40">
          ...
          <div className="hidden md:flex items-center gap-8">
            {['Features','Pricing','Testimonials','FAQ'].map(item => (
              <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-bold text-[#7ab3e8] hover:text-white transition-colors">{item}</a>
            ))}
          </div>
          <div className="flex items-center gap-3">
            <button onClick={onGetStarted}
              className="hidden md:block bg-[#134589] text-white px-5 py-2.5 rounded-xl font-bold text-sm hover:bg-[#1a5cad] transition-all shadow-lg shadow-[#134589]/30">
              Get Started Free
            </button>
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden p-2 text-[#7ab3e8]">
              <Menu size={20} />
            </button>
          </div>
        </div>
        {mobileMenuOpen && (
          <div className="md:hidden bg-[#041628] border-t border-[#0e3a72] px-6 py-4 space-y-3">
            {['Features','Pricing','Testimonials','FAQ'].map(item => (
              <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setMobileMenuOpen(false)} className="block text-sm font-bold py-2 text-[#aaccf2]">{item}</a>
            ))}
            <button onClick={onGetStarted} className="w-full bg-[#134589] text-white py-3 rounded-xl font-bold text-sm mt-2">Get Started Free</button>
          </div>
        )}
      </nav>
      ...
      <section className="pt-32 pb-24 px-6 relative overflow-hidden">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute -top-40 -right-40 w-[600px] h-[600px] bg-[#134589]/20 rounded-full blur-3xl" />
          <div className="absolute top-1/2 -left-40 w-80 h-80 bg-[#0e3a72]/30 rounded-full blur-3xl" />
        ...
        <div className="max-w-5xl mx-auto text-center relative">
          <div className="inline-flex items-center gap-2 bg-[#0a2d5a] text-[#7ab3e8] px-4 py-2 rounded-full text-xs font-bold uppercase tracking-widest mb-8 border border-[#134589]">
          ...
          <p className="text-xl md:text-2xl font-medium max-w-2xl mx-auto mb-12 text-[#7ab3e8]">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
            <button onClick={onGetStarted}
              className="bg-[#134589] text-white px-10 py-5 rounded-2xl font-black text-lg flex items-center gap-3 hover:bg-[#1a5cad] transition-all shadow-2xl shadow-[#134589]/40 hover:scale-105 active:scale-95">
              Start for Free <ArrowRight size={22} />
            </button>
            <button onClick={onGetStarted}
              className="px-10 py-5 rounded-2xl font-black text-lg flex items-center gap-3 border-2 border-[#134589] text-[#aaccf2] hover:bg-[#062040] transition-all hover:scale-105">
              <Camera size={20} className="text-[#4d93d9]" /> AI Demo →
            </button>
          </div>
          <div className="flex flex-wrap items-center justify-center gap-6 text-sm font-bold text-[#4d7291]">
            <span className="flex items-center gap-2"><Shield size={14} className="text-emerald-400" /> LSK Compliant</span>
            <span className="flex items-center gap-2"><Lock size={14} className="text-[#4d93d9]" /> Bank-grade Encryption</span>
      ...
      {/* ── Stats ── */}
      <section className="py-16 px-6 bg-[#041628] border-y border-[#0e3a72]">
        <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8">
          {STATS.map((stat, i) => (
            <div key={i} className="text-center">
              <div className="flex justify-center mb-3">
                <div className="w-10 h-10 bg-[#0a2d5a] border border-[#134589] rounded-2xl flex items-center justify-center">
                  <stat.icon size={20} className="text-[#4d93d9]" />
                </div>
              </div>
              <div className="text-3xl md:text-4xl font-black tracking-tighter mb-1 text-white">{stat.value}</div>
              <div className="text-xs font-bold uppercase tracking-widest text-[#4d7291]">{stat.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="features" className="py-24 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <div className="text-xs font-black uppercase tracking-widest mb-4 text-[#4d93d9]">Everything You Need</div>
            ...
            <p className="text-xl max-w-2xl mx-auto text-[#7ab3e8]">Eight powerful tools, seamlessly integrated. No juggling between apps.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {FEATURES.map((feature, i) => (
              <div key={i}
                className="group p-6 rounded-3xl border border-[#0e3a72] bg-[#041628] hover:border-[#134589] hover:bg-[#062040] transition-all duration-300 hover:shadow-2xl hover:shadow-[#134589]/10 cursor-pointer hover:-translate-y-1"
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full bg-[#0a2d5a] text-[#4d93d9] border border-[#134589]/50">{feature.badge}</span>
                </div>
                <p className="text-sm leading-relaxed text-[#4d7291]">{feature.desc}</p>
                <div className="mt-4 flex items-center gap-1 text-xs font-bold text-[#4d93d9] opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      {/* ── How it Works ── */}
      <section className="py-24 px-6 bg-[#041628] border-y border-[#0e3a72]">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <div className="text-xs font-black uppercase tracking-widest mb-4 text-[#4d93d9]">Simple Workflow</div>
            <h2 className="text-5xl font-black tracking-tighter text-white">From rubber stamp<br />to signed document</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { step:'01', title:'Design or Digitize',  desc:'Build your stamp from scratch or photograph your existing one — AI recreates it perfectly.',               icon:PenTool,      color:'bg-[#134589]' },
              ...
              <div key={i} className="relative p-8 rounded-3xl bg-[#020b18] border border-[#0e3a72] hover:border-[#134589] transition-colors">
                <div className={`absolute -top-4 -left-4 w-10 h-10 ${step.color} rounded-2xl flex items-center justify-center shadow-lg`}>
                  <step.icon size={18} className="text-white" />
                </div>
                <div className="text-6xl font-black tracking-tighter mb-4 text-[#062040]">{step.step}</div>
                <h3 className="text-xl font-black mb-3 text-white">{step.title}</h3>
                <p className="text-sm leading-relaxed text-[#4d7291]">{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="testimonials" className="py-24 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16">
            <div className="text-xs font-black uppercase tracking-widest mb-4 text-[#4d93d9]">Trusted by Professionals</div>
            <h2 className="text-5xl font-black tracking-tighter text-white">What our users say</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {TESTIMONIALS.map((t, i) => (
              <div key={i} className="p-8 rounded-3xl border border-[#0e3a72] bg-[#041628] hover:border-[#134589] transition-colors">
                <div className="flex gap-1 mb-6">
                  {Array.from({length: t.stars}).map((_, j) => <Star key={j} size={16} className="fill-yellow-400 text-yellow-400" />)}
                </div>
                <p className="text-sm leading-relaxed mb-8 text-[#aaccf2]">"{t.text}"</p>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] font-bold text-[#4d7291]">{t.role}</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* ── Pricing ── */}
      <section id="pricing" className="py-24 px-6 bg-[#041628] border-y border-[#0e3a72]">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-16">
            <div className="text-xs font-black uppercase tracking-widest mb-4 text-[#4d93d9]">Transparent Pricing</div>
            <h2 className="text-5xl font-black tracking-tighter text-white">Start free. Scale as you grow.</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {PRICING.map((plan, i) => (
              <div key={i} className={`relative p-8 rounded-3xl border-2 transition-all ${plan.highlight ? 'border-[#134589] bg-[#062040] shadow-2xl shadow-[#134589]/20' : 'border-[#0e3a72] bg-[#020b18]'}`}>
                {plan.highlight && (
                ...
                <div className="mb-6">
                  ...
                  <p className="text-sm text-[#4d7291]">{plan.desc}</p>
                </div>
                <div className="mb-8">
                  <span className="text-4xl font-black tracking-tighter text-white">{plan.price}</span>
                  <span className="text-sm font-bold text-[#4d7291]">{plan.period}</span>
                </div>
                <button onClick={onGetStarted}
                  className={`w-full py-4 rounded-2xl font-black text-sm transition-all mb-8 ${plan.highlight ? 'bg-[#134589] text-white hover:bg-[#1a5cad]' : 'bg-[#0a2d5a] text-[#aaccf2] hover:bg-[#134589] hover:text-white'}`}>
                ...
                <div className="space-y-3">
                  {plan.features.map((f, j) => (
                    <div key={j} className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-[#aaccf2]">{f}</span>
                    </div>
                  ))}
                  {plan.missing.map((f, j) => (
                    <div key={j} className="flex items-center gap-3 opacity-30">
                      <X size={14} className="flex-shrink-0 text-[#4d7291]" />
                      <span className="text-sm font-medium text-[#4d7291]">{f}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <section id="faq" className="py-24 px-6">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-16">
            <div className="text-xs font-black uppercase tracking-widest mb-4 text-[#4d93d9]">Got Questions?</div>
            <h2 className="text-5xl font-black tracking-tighter text-white">Frequently Asked</h2>
          </div>
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <div key={i} className={`rounded-2xl border overflow-hidden transition-all ${openFaq === i ? 'border-[#134589] bg-[#041628]' : 'border-[#0e3a72] bg-[#020b18] hover:border-[#134589]/50'}`}>
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)} className="w-full flex items-center justify-between p-6 text-left">
                  <span className="font-bold text-sm pr-4 text-white">{faq.q}</span>
                  <ChevronDown size={18} className={`flex-shrink-0 transition-transform text-[#4d7291] ${openFaq === i ? 'rotate-180' : ''}`} />
                </button>
                {openFaq === i && <div className="px-6 pb-6 text-sm leading-relaxed text-[#7ab3e8]">{faq.a}</div>}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative overflow-hidden bg-gradient-to-br from-[#0e3a72] via-[#134589] to-[#1a5cad] rounded-[3rem] p-12 md:p-16 text-center border border-[#1a5cad]">
            ...
            <div className="relative">
              <Award size={48} className="mx-auto mb-6 text-[#aaccf2]" />
              <h2 className="text-4xl md:text-5xl font-black tracking-tighter mb-6 text-white">Ready to go digital?</h2>
              <p className="text-xl text-[#aaccf2] mb-10 max-w-xl mx-auto">Join 4,200+ professionals who trust Tomo for their digital stamp and document authority.</p>
              <button onClick={onGetStarted}
      ...
      {/* ── Footer ── */}
      <footer className="border-t border-[#0e3a72] bg-[#020b18] py-12 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <p className="text-[11px] font-bold uppercase tracking-widest text-[#224260]">
            © 2024 JijiTechy Innovations · LSK Standards Applied · Nairobi, Kenya
          </p>
          <div className="flex gap-6 text-sm font-bold text-[#4d7291]">
            <a href="#" className="hover:text-[#4d93d9] transition-colors">Privacy</a>
            <a href="#" className="hover:text-[#4d93d9] transition-colors">Terms</a>
            <a href="#" className="hover:text-[#4d93d9] transition-colors">Contact</a>
          </div>
        </div>
      </footer>
    </div>
