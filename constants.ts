
import { StampShape, StampTemplate, BorderStyle, StampConfig } from './types';

export const TEMPLATES: StampTemplate[] = [
  // THE CARISON SERIES (High Accuracy)
  {
    id: 'carison-01',
    name: 'Carison Limited (The Original)',
    category: 'Business',
    shape: StampShape.OVAL,
    primaryText: 'CARISON LIMITED.',
    innerTopText: 'For Rubber Stamps & Company Seals',
    centerText: '31 MAR 2017',
    innerBottomText: 'Tel: 0722174777',
    secondaryText: 'P.O. Box 15181 - 00400, NAIROBI, KENYA',
    borderColor: '#0000FF',
    secondaryColor: '#FF0000',
    fontFamily: 'Crimson Pro',
    showStars: true,
    showInnerLine: true,
    innerLineOffset: 12,
    wetInk: true
  },
  
  // HELMARC GROUP (High Accuracy)
  {
    id: 'helmarc-01',
    name: 'Helmarc Brands (Rectangle Received)',
    category: 'Financial',
    shape: StampShape.RECTANGLE,
    primaryText: 'HELMARC BRANDS KENYA',
    centerText: 'RECEIVED',
    centerSubText: '27 JULY 2020',
    secondaryText: 'P.O. Box 4417 - 00100 NAIROBI',
    borderColor: '#1e3a8a',
    secondaryColor: '#991b1b',
    fontFamily: 'Inter',
    showInnerLine: true,
    innerLineOffset: 8
  },
  {
    id: 'helmarc-02',
    name: 'Helmarc Brands (Rectangle PAID)',
    category: 'Financial',
    shape: StampShape.RECTANGLE,
    primaryText: 'HELMARC BRANDS KENYA',
    centerText: 'PAID',
    centerSubText: '27 JULY 2020',
    secondaryText: 'P.O. Box 4417 - 00100 NAIROBI',
    borderColor: '#1e3a8a',
    secondaryColor: '#991b1b',
    fontFamily: 'Inter'
  },
  {
    id: 'helmarc-03',
    name: 'Helmarc Brands (Oval Received)',
    category: 'Financial',
    shape: StampShape.OVAL,
    primaryText: 'HELMARC BRANDS KENYA',
    centerText: 'RECEIVED',
    centerSubText: '27 JULY 2020',
    secondaryText: 'P.O. Box 4417 - 00100, NAIROBI',
    borderColor: '#1e3a8a',
    secondaryColor: '#991b1b',
    fontFamily: 'Inter'
  },
  {
    id: 'helmarc-04',
    name: 'Helmarc Brands (Round Received)',
    category: 'Financial',
    shape: StampShape.ROUND,
    primaryText: 'HELMARC BRANDS KENYA',
    centerText: 'RECEIVED',
    centerSubText: '27 JULY 2020',
    secondaryText: 'P.O. Box 4417 - 00100 NAIROBI',
    borderColor: '#1e3a8a',
    secondaryColor: '#991b1b',
    fontFamily: 'Crimson Pro'
  },
  {
    id: 'helmarc-05',
    name: 'Helmarc Mabati (Received)',
    category: 'Business',
    shape: StampShape.RECTANGLE,
    primaryText: 'HELMARC MABATI FACTORY LTD.',
    centerText: 'RECEIVED',
    centerSubText: '02 DEC 2034',
    secondaryText: 'P.O. BOX 5687-00100, NAIROBI',
    borderColor: '#1e3a8a',
    secondaryColor: '#991b1b',
    fontFamily: 'Inter',
    showDateLine: true
  },

  // COASTAMPS SERIES
  {
    id: 'coast-01',
    name: 'Coastamps Kenya Solutions',
    category: 'Business',
    shape: StampShape.OVAL,
    primaryText: 'COASTAMPS KENYA SOLUTIONS',
    centerText: '08 JAN 2021',
    secondaryText: 'P.O. Box 80100 - 80100, MOMBASA',
    borderColor: '#000080',
    secondaryColor: '#FF0000',
    fontFamily: 'Crimson Pro',
    showStars: true
  },

  // LEGAL & ADVOCATES
  {
    id: 'adv-kip-01',
    name: 'Michael Kipchirchir Advocate',
    category: 'Legal',
    shape: StampShape.RECTANGLE,
    primaryText: 'MICHAEL KIPCHIRCHIR',
    centerText: 'ADVOCATE',
    secondaryText: 'P.O. Box 39887 - 00200, NAIROBI',
    borderColor: '#111827',
    fontFamily: 'Crimson Pro'
  },
  {
    id: 'adv-hil-01',
    name: 'H.S. Hillary Advocate',
    category: 'Legal',
    shape: StampShape.ROUND,
    primaryText: 'H.S. HILLARY',
    innerTopText: 'ADVOCATE &',
    innerBottomText: 'COMMISSIONER FOR OATHS',
    secondaryText: 'P.O. BOX 8333 - 00100, NAIROBI',
    borderColor: '#111827',
    fontFamily: 'Crimson Pro'
  },
  {
    id: 'adv-kam-01',
    name: 'Michael Kamau & Assoc.',
    category: 'Legal',
    shape: StampShape.RECTANGLE,
    primaryText: 'MICHAEL KAMAU & ASSOCIATES ADVOCATES',
    centerText: 'RECEIVED',
    centerSubText: '06 JUL 2034',
    secondaryText: 'P.O. Box 87480 - 00100, NAIROBI',
    borderColor: '#1e3a8a',
    secondaryColor: '#991b1b',
    fontFamily: 'Inter',
    showSignatureLine: true
  },

  // AMUSADE & OFFICIAL
  {
    id: 'amu-gaf-01',
    name: 'Amusade GAF Foundation',
    category: 'Official',
    shape: StampShape.ROUND,
    primaryText: 'AMUSADE GAF FOUNDATION',
    centerText: 'RECEIVED',
    centerSubText: 'In Good Condition',
    secondaryText: 'GOODS RECEIVED IN GOOD CONDITION',
    borderColor: '#1e3a8a',
    fontFamily: 'Inter',
    showSignatureLine: true
  },
  {
    id: 'winsome-01',
    name: 'Winsome Spa (Diamond Plaza)',
    category: 'Business',
    shape: StampShape.ROUND,
    primaryText: 'WINSOME SPA ★ OFFICIAL ★',
    centerText: '02 DEC 2034',
    secondaryText: '3rd Parklands, Diamond Plaza II, Nairobi',
    borderColor: '#581c87',
    fontFamily: 'Inter'
  },
  {
    id: 'status-app-red',
    name: 'APPROVED Red Round',
    category: 'Official',
    shape: StampShape.ROUND,
    primaryText: 'APPROVED',
    centerText: '10 MAY 2023',
    secondaryText: 'Thank you for your business',
    borderColor: '#B91C1C',
    fontFamily: 'Inter',
    showSignatureLine: true,
    showStars: true
  },
  {
    id: 'status-app-blue',
    name: 'APPROVED Blue Oval',
    category: 'Official',
    shape: StampShape.OVAL,
    primaryText: 'APPROVED',
    centerText: '10 MAY 2023',
    secondaryText: 'Thank you for your business',
    borderColor: '#1e40af',
    fontFamily: 'Inter',
    showSignatureLine: true,
    showStars: true
  },
  {
    id: 'office-invoice-01',
    name: 'Official Invoice Stamp',
    category: 'Financial',
    shape: StampShape.RECTANGLE,
    primaryText: 'INVOICE PROCESSED',
    centerText: 'PAID',
    secondaryText: 'ACCOUNTS DEPARTMENT',
    borderColor: '#15803d',
    fontFamily: 'Inter',
    primaryBold: true,
    centerBold: true
  },
  {
    id: 'office-memo-01',
    name: 'Internal Memo Stamp',
    category: 'Official',
    shape: StampShape.SQUARE,
    primaryText: 'INTERNAL MEMO',
    centerText: 'CONFIDENTIAL',
    secondaryText: 'DO NOT CIRCULATE',
    borderColor: '#991b1b',
    fontFamily: 'Inter',
    primaryBold: true,
    centerBold: true
  },
  {
    id: 'office-received-01',
    name: 'Office Received (Simple)',
    category: 'Official',
    shape: StampShape.ROUND,
    primaryText: 'OFFICE OF THE DIRECTOR',
    centerText: 'RECEIVED',
    secondaryText: 'NAIROBI HEADQUARTERS',
    borderColor: '#1e3a8a',
    fontFamily: 'Crimson Pro',
    showDateLine: true
  },
  {
    id: 'fin-audit-01',
    name: 'Audit Verified',
    category: 'Financial',
    shape: StampShape.OVAL,
    primaryText: 'AUDIT DEPARTMENT',
    centerText: 'VERIFIED',
    secondaryText: 'COMPLIANCE CHECKED',
    borderColor: '#111827',
    fontFamily: 'Inter',
    showInnerLine: true,
    innerLineOffset: 10
  },
  {
    id: 'fin-tax-01',
    name: 'Tax Compliance Stamp',
    category: 'Financial',
    shape: StampShape.RECTANGLE,
    primaryText: 'TAX COMPLIANCE VERIFIED',
    centerText: 'KRA COMPLIANT',
    centerSubText: 'FY 2024/2025',
    secondaryText: 'REVENUE AUTHORITY DIVISION',
    borderColor: '#991b1b',
    fontFamily: 'Inter',
    primaryBold: true
  },
  {
    id: 'off-dispatch-01',
    name: 'Dispatch Center',
    category: 'Official',
    shape: StampShape.ROUND,
    primaryText: 'LOGISTICS & DISPATCH CENTER',
    centerText: 'DISPATCHED',
    centerSubText: 'VIA COURIER',
    secondaryText: 'GATE PASS REQUIRED',
    borderColor: '#15803d',
    fontFamily: 'Inter',
    showDateLine: true
  },
  {
    id: 'off-certified-01',
    name: 'Certified True Copy',
    category: 'Legal',
    shape: StampShape.RECTANGLE,
    primaryText: 'CERTIFIED TRUE COPY OF THE ORIGINAL',
    centerText: 'Michael Kamau',
    centerSubText: 'ADVOCATE',
    secondaryText: 'COMMISSIONER FOR OATHS',
    borderColor: '#1e3a8a',
    fontFamily: 'Crimson Pro',
    showSignatureLine: true
  },
  {
    id: 'off-secret-01',
    name: 'Top Secret / Classified',
    category: 'Official',
    shape: StampShape.SQUARE,
    primaryText: 'TOP SECRET',
    centerText: 'CLASSIFIED',
    secondaryText: 'EYES ONLY',
    borderColor: '#000000',
    fontFamily: 'Inter',
    primaryBold: true,
    centerBold: true,
    doubleBorder: true,
    doubleBorderOffset: 8,
    doubleBorderThickness: 2,
    doubleBorderIsOuter: false
  },
  {
    id: 'fin-invoice-02',
    name: 'Commercial Invoice',
    category: 'Financial',
    shape: StampShape.RECTANGLE,
    primaryText: 'COMMERCIAL INVOICE',
    centerText: 'VALIDATED',
    secondaryText: 'EXPORT DIVISION',
    borderColor: '#1e3a8a',
    fontFamily: 'Inter',
    primaryBold: true
  },
  {
    id: 'off-memo-02',
    name: 'Executive Memo',
    category: 'Official',
    shape: StampShape.OVAL,
    primaryText: 'EXECUTIVE MEMORANDUM',
    centerText: 'ACTION REQUIRED',
    secondaryText: 'OFFICE OF THE CEO',
    borderColor: '#991b1b',
    fontFamily: 'Crimson Pro',
    showStars: true
  },
  {
    id: 'fin-audit-02',
    name: 'Internal Audit',
    category: 'Financial',
    shape: StampShape.ROUND,
    primaryText: 'INTERNAL AUDIT UNIT',
    centerText: 'PASSED',
    secondaryText: 'COMPLIANCE VERIFIED',
    borderColor: '#15803d',
    fontFamily: 'Inter',
    showInnerLine: true
  }
];

export const DEFAULT_CONFIG: StampConfig = {
  shape: StampShape.ROUND,
  primaryText: 'YOUR COMPANY NAME HERE',
  secondaryText: 'LOCATION OR ADDRESS',
  innerTopText: '',
  innerBottomText: '',
  centerText: 'OFFICIAL SEAL',
  centerSubText: '',
  fontSize: 22,
  letterSpacing: 2,
  letterStretch: 1,
  borderColor: '#1e3a8a',
  borderWidth: 3,
  borderOffset: 0,
  borderStyle: BorderStyle.SINGLE,
  rotation: 0,
  width: 600,
  height: 600,
  stretchX: 1,
  stretchY: 1,
  fontFamily: 'Crimson Pro',
  showSignatureLine: false,
  showDateLine: false,
  showStars: false,
  showInnerLine: false,
  innerLineOffset: 15,
  innerLineWidth: 2,
  innerTextColor: '#1e3a8a',
  innerTextSize: 14,
  innerTextIntensity: 1,
  starCount: 2,
  starSize: 20,
  starOffset: 0,
  distressLevel: 0.1,
  isVintage: false,
  wetInk: false,
  logoUrl: null,
  embeddedSignatureUrl: null,
  showEmbeddedSignature: false,
  customElements: [],
  previewBg: 'default',
  primaryFontFamily: 'Crimson Pro',
  secondaryFontFamily: 'Crimson Pro',
  innerTopFontFamily: 'Crimson Pro',
  innerBottomFontFamily: 'Crimson Pro',
  centerFontFamily: 'Crimson Pro',
  centerSubFontFamily: 'Crimson Pro',
  primaryXOffset: 0,
  primaryYOffset: 0,
  secondaryXOffset: 0,
  secondaryYOffset: 0,
  innerTopXOffset: 0,
  innerTopYOffset: 0,
  innerBottomXOffset: 0,
  innerBottomYOffset: 0,
  centerXOffset: 0,
  centerYOffset: 0,
  centerSubXOffset: 0,
  centerSubYOffset: 0,
  statusText: '',
  statusFontSize: 40,
  statusFontFamily: 'Inter',
  statusX: 0,
  statusY: 0,
  statusRotation: 0,
  doubleBorder: false,
  doubleBorderOffset: 5,
  doubleBorderThickness: 1,
  doubleBorderIsOuter: false,
  primaryColor: '#1e3a8a',
  secondaryColor: '#1e3a8a',
  innerTopColor: '#1e3a8a',
  innerBottomColor: '#1e3a8a',
  centerColor: '#1e3a8a',
  centerSubColor: '#1e3a8a',
  statusColor: '#1e3a8a',
  primaryFontSize: 22,
  secondaryFontSize: 22,
  innerTopFontSize: 14,
  innerBottomFontSize: 14,
  centerFontSize: 22,
  centerSubFontSize: 18,
  primaryBold: false,
  secondaryBold: false,
  innerTopBold: false,
  innerBottomBold: false,
  centerBold: false,
  centerSubBold: false,
  statusBold: true,
  signatureX: 0,
  signatureY: 0,
  signatureScale: 1,
  lockDragAxis: 'none',
  doubleBorderColor: '#1e3a8a',
  doubleBorderStyle: BorderStyle.SINGLE
};

export const COLORS = [
  { name: 'Onyx Black', value: '#111827' },
  { name: 'Royal Blue', value: '#1e3a8a' },
  { name: 'Pure Blue', value: '#0000FF' },
  { name: 'Crimson Red', value: '#991b1b' },
  { name: 'Standard Red', value: '#FF0000' },
  { name: 'Forest Green', value: '#15803d' },
  { name: 'Deep Purple', value: '#581c87' },
];

export const FONTS = [
  { name: 'Classic Serif', value: 'Crimson Pro' },
  { name: 'Modern Sans', value: 'Inter' },
  { name: 'Industrial Slab', value: 'Courier New' },
  { name: 'Vintage Mono', value: 'monospace' },
];
